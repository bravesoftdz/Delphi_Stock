...
begin
 ...
 {
  // E7-1,2 軋單 (只判斷第一點)
  if GetQty= 0 then begin
   LastClose:= SK.Ref / 100;
   ThisOpen:= SK.Open / 100;

   if (fmChungYi.ListV_Interest.Items.Count > 0) then  // 有庫存值
   begin
    if fmChungYi.ListV_Interest.Items[0].SubItems.strings[1]= 'B' then
    begin
     if (LastClose - ThisOpen) > 0 then
     begin
      sStrategy.E4_1_Weight:= 1;// 不同向
      LastInventory:= True;
     end else LastInventory:= False;
     if ThisOpen < LastLow then sStrategy.E4_2_Weight:= 1;//超出昨日的高點/低點 則出場 (5-4-2)
    end;

    if fmChungYi.ListV_Interest.Items[0].SubItems.strings[1]= 'S' then
    begin
     if (ThisOpen - LastClose) > 0 then
     begin
      sStrategy.E4_1_Weight:= 1;// 不同向
      LastInventory:= True;
     end else LastInventory:= False;
     if ThisOpen > LastHigh then sStrategy.E4_2_Weight:= 1;//超出昨日的高點/低點 則出場 (5-4-2)
    end;
   end;

   // 以今天開盤, 寫入昨天的趨勢
   if ThisOpen > LastClose then
   begin
    FigureStr:= IntToStr(Trunc((ThisOpen - LastClose)/25) + 1);  // 今日開盤在圖形落點區間
    DataModule1.asqQU_Figure.SQL.Text:= 'update Figure set NextUpDown="U" where TradeDate="' + LastDate + '"'
   end else
   if ThisOpen < LastClose then
   begin
    FigureStr:= IntToStr(Trunc((ThisOpen - LastClose)/25) - 1);
    DataModule1.asqQU_Figure.SQL.Text:= 'update Figure set NextUpDown="D" where TradeDate="' + LastDate + '"'
   end else
   begin
    FigureStr:= '0';  // ThisOpen = LastClose
    DataModule1.asqQU_Figure.SQL.Text:= 'update Figure set NextUpDown="E" where TradeDate="' + LastDate + '"';
   end;
   DataModule1.asqQU_Figure.Open;
  end;
  }
  if PrepareData then  // 是否處理之前下載資料
  begin
   PrepareData:= False;
   ...
   // 畫成交價範圍
//   fmQuote.MainChart.LeftAxis.Minimum:= Round(LastClose*0.93);
//   fmQuote.MainChart.LeftAxis.Maximum:= Round(LastClose*1.07);
//     fmQuote.MainChart.LeftAxis.Maximum:= 12000;
   GetOriginalData();
  end;
  ...
  for I:= GetQty to ToQty do begin
   ...
   if iSt = 0 then begin
     ...
     // fmQuote.Memo2.Lines.Add(IntToStr(I));
      TotalQty:= TotalQty + 1;
      if(TK.nClose= 0) then
        continue;
      ...
      begin
       ...
       // if (Round(CurrMin* 86400) mod 3600 <= 10) and ((CurrMin - PhotoTime) > 1/8640) then
        if ((Copy(TickTime, 4, 2)= '45') or (Copy(TickTime, 1, 5)= '09:15') or (Copy(TickTime, 1, 5)= '13:40'))
           and (Copy(TickTime, 1, 5)<> '13:45') then
        begin
      ...
      if AddStrTrue and (TickTime <> LockTimeStr) then
      begin
       A13_14_Open:= True;
       ...
       begin
        sStrategy.D3_K_High:= GetMax(HighList, StrToInt(fmChungYi.dbeD3.Text)-1, 'End');
        sStrategy.D3_K_Low:= GetMin(LowList, StrToInt(fmChungYi.dbeD3.Text)-1, 'End');
       end;
   ...
   //======================================================================================
      ...
      if NowHigh < TK.nClose / iDot then
      begin
      ...
      //A2_3 取得最近五檔的成交量
      if (fmChungYi.dbckA2_3.Checked) then // 若 A2_1 有勾選
      begin
       fmChungYi.lbBuy.Caption:= FloatToStr(A2_BestBuyQty);
       fmChungYi.lbSell.Caption:= FloatToStr(A2_BestSellQty);
       if (fmChungYi.ListV_Interest.Items.Count > 0) and A2Pass then  // 需 A2_1成立
       begin
        if fmChungYi.ListV_Interest.Items[0].SubItems.strings[1]= 'B' then
         if A2_BestBuyQty < A2_BestSellQty then
          A2Pass:= True
         else A2Pass:= False;
        if fmChungYi.ListV_Interest.Items[0].SubItems.strings[1]= 'S' then
         if A2_BestBuyQty > A2_BestSellQty then
       ...
       end;// else if A2_BestBuyQty < A2_BestSellQty -----> Inventory
      end;
      ...
      begin
       ...
       if sStrategy.A2_5_1S + sStrategy.A2_5_2S + sStrategy.A2_5_3S >= 2 then
        SellOK:= True
        else SellOK:= False;
       if sStrategy.A2_5_1B + sStrategy.A2_5_2B + sStrategy.A2_5_3B >= 2 then
        BuyOK:= True
        else BuyOK:= False;
      end else begin
       SellOK:= True;
...
//   fmQuote.MainChart.LeftAxis.Maximum:= Round(LastClose*1.07);
  ...
  for I:= GetQty to ToQty do
  begin
   iSt := iSt + SKQuoteLib_GetTick(sMarket, sIndex, I, TK);
   if iSt = 0 then
   begin
      TotalQty:= TotalQty + 1;
      TickTime:= TimeTransfer(IntToStr(TK.Time));
      // 每分鐘
      if Copy(FormatDateTime('hh:mm:ss', CurrMin), 1, 5) <> Copy(TickTime, 1, 5) then
      begin
       CurrMin:= StrTotime(TickTime);  // 更新時間
   ...
   //======================================================================================
       // 下載所有資料
      AllSQL:= fmQuote.cbbStockNO.Text + ',' + DateToStr(fmQuote.dtpDate.Date) + ',' + TickTime + ','
        + FormatFloat('0.000', TK.Bid / iDot) + ', ' +
        FormatFloat('0.000', TK.Ask / iDot) + ', ' + FormatFloat('0.000', TK.Close / iDot) + ', ' +
        IntToStr(TK.Qty) + ', ' + FloatToStr(Ave5P);
      LastSN:= LastSN + 1;
      AllSQLList.Add(IntToStr(LastSN) + ',' + AllSQL);
      // 最後一秒抓取資料的判讀
      if Ticktime= '13:44:59' then
      ...
      // 中間每一筆均更新
      if CloseP= 0 then OpenP:= TK.Close / iDot; // 成交價 (即開盤價)
      CloseP:= TK.Close / iDot; // 成交價 (即收盤價)
      MinuteQty:= MinuteQty + TK.Qty; // 每分鐘累積成交量
      ...
      if NowHigh < TK.Close / iDot then
      begin
       NowHigh:= TK.Close / iDot;
       HourHigh:= NowHigh;
       HourHTime:= TickTime;
      end;

      if NowLow = 0 then NowLow:= TK.Close / iDot
      else if NowLow > TK.Close / iDot then
           begin
            NowLow:= TK.Close / iDot;
            HourLow:= NowLow;
            HourLTime:= TickTime;
           end;
...
// iSKQStatus := SKQuoteLib_GetKLine(GiveStockNO, 3); // 取得日線
 ...
 GetOpenOrder();  // 以資料庫方式  獲取未平倉內容 (須刪除)
 fmQuote.lbBalance.Caption:= '當日損益: ' + FloatToStr(GetBalance());
 // 取得過去下單記錄 10 筆
 DataModule1.asqQU_Temp.SQL.Text:= 'select Price from RecordMsg order by TradeDate, TradeTime Desc limit 10';
 DataModule1.asqQU_Temp.Open;
 DataModule1.asqQU_Temp.First;
 if DataModule1.asqQU_Temp.RecordCount > 0 then
  for I := 0 to DataModule1.asqQU_Temp.RecordCount do
  begin
   OrderList.Add(DataModule1.asqQU_Temp.FieldByName('Price').Text);
...
//  SKQuoteLib1.SKQuoteLib_RequestServerTime();
 // TimerSystem.Enabled:= False; // 只執行一次即可
 {
 try
//  SystemTime:= SKOSQuoteLib1.SKOSQuoteLib_RequestServerTime();
  SystemTime:= SKQuoteLib1.SKQuoteLib_RequestServerTime();
 // TimerSystem.Enabled:= False; // 只執行一次即可
...
//  Test:= StrtoTime(TickTimeStr);
 ...
 {
 MainChart.LeftAxis.Maximum:= nowHigh * 1.3;
