...
uses Windows, Messages, SysUtils, Controls, Classes, DBClient, DB, Dialogs,
 ...
 function A4_2FloatCheck(): Boolean;
 procedure A4_Module();
 procedure ReSetK(U_or_D: String);  // K 觸均 Reset

implementation

uses Quote, Public_Variant, ChungYi_Main, DMRecord, StringList_Fun;
...
begin
 ...
 begin
  ...
  if (fmChungYi.dbrgB2or3.ItemIndex= 1) and (fmChungYi.dbeB1.Text <>'') then
  begin
   sStrategy.B_HighU:= SetData.P_Highest + DataModule1.asqQU_Param.FieldByName('B3').AsExtended;
   sStrategy.B_HighD:= SetData.P_Highest - DataModule1.asqQU_Param.FieldByName('B3').AsExtended;
   sStrategy.B_LowU:= SetData.P_Lowest + DataModule1.asqQU_Param.FieldByName('B3').AsExtended;
   sStrategy.B_LowD:= SetData.P_Lowest - DataModule1.asqQU_Param.FieldByName('B3').AsExtended;
  end;
...
// A4-2 在下單前判定是否為  Floated
function A4_2FloatCheck(): Boolean;
begin
    // A4-2  高低差 模組
    if (fmChungYi.dbrA1.ItemIndex= 2) and (fmChungYi.dbcbA4_23Check.Checked) then
    begin
     if (fmChungYi.dbrgA4_2or3.ItemIndex= 0) then
      if (fmChungYi.dbeA4_2.Text <> '') and (fmChungYi.dbeA4_2_1.Text <> '')
       and (StrToInt(fmChungYi.dbeA4_2.Text) <= OrderList.Count) then
      begin
       A4_2Max:= GetMax(OrderList, StrToInt(fmChungYi.dbeA4_2.Text), 'End');
       A4_2Min:= GetMin(OrderList, StrToInt(fmChungYi.dbeA4_2.Text), 'End');

       if (A4_2Max - A4_2Min) <= StrToInt(fmChungYi.dbeA4_2_1.Text) then
        Result:= True
       else Result:= False;
      end;
    end;
...
{      A4_2Max:= GetMax(HighList, StrToInt(fmChungYi.dbeA4_3_1.Text), 'End');
    ...
    if fmChungYi.dbcbA4_23Check.Checked and (fmChungYi.dbrgA4_2or3.ItemIndex= 0) then
     if (fmChungYi.dbeA4_2.Text <> '') and (fmChungYi.dbeA4_2_1.Text <> '') then
      A4_2Float:= A4_2FloatCheck();
...
begin
 ...
 // 寫入今日的圖形記錄
   DataModule1.asqQU_Figure.SQL.Text:= 'insert into Figure values("' + IntToStr(FigureSN + 1) + '","'
     ...
     + FloatToStr(LastClose) +'","' + FloatToStr(ThisOpen) + '","' + FigureStr + '","")';
...
begin
  ...
  OrderList.SaveToFile('OrderList.txt');
